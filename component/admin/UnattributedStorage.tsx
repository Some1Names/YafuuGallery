...
export default function UnattributedStorage({ objects }: UnattributedStorageProps) {
  ...
  return (
    <div className="border border-[#050505] rounded-md bg-[#1b1a1c]/60 mb-10">
      <button
        type="button"
      ...
      >
        <div className="text-sm text-[#ece6d8]">
          ...
          <span className="text-[#b6b0a2]">
            unattributed across {objects.length} object{objects.length === 1 ? "" : "s"}
          </span>
        </div>
        <ChevronDown className={`w-4 h-4 text-[#6b655e] transition-transform duration-200 shrink-0 ${isExpanded ? "rotate-180" : ""}`} />
      </button>

      {isExpanded && (
        <div className="border-t border-[#050505] p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1.5 max-h-80 overflow-y-auto">
            {objects.map((o) => (
              <div
                key={o.key}
                className="flex items-center justify-between gap-3 text-xs bg-[#0a0a0a]/40 rounded px-3 py-2"
              >
                <span className="text-[#b6b0a2] truncate font-mono">{o.key}</span>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-[#6b655e]">{formatBytes(o.size)}</span>
                  <button
                    type="button"
                    onClick={() => handleDeleteOne(o.key)}
                    disabled={deletingKeys.has(o.key) || isDeletingAll}
                    className="text-[#9c1d25] hover:bg-[#9c1d25]/10 rounded p-1 disabled:opacity-40 transition-colors duration-200"
          ...
          <button
            ...
            className="self-end text-xs px-3 py-1.5 border border-[#9c1d25]/50 rounded text-[#9c1d25] hover:bg-[#9c1d25]/10 disabled:opacity-50 transition-colors duration-200"
          >
